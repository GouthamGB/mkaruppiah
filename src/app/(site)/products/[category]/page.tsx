...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const allProducts = await sanityFetch<Category[]>({
    query: `*[_type == "product"] | order(coalesce(orderRank, order, 999) asc, _createdAt asc) { _id, id, name, description, "slug": slug.current }`,
  });
  ...
  const subcategories = category
    ? await sanityFetch<Subcategory[]>({
        ...
        )] | order(coalesce(orderRank, order, 999) asc, _createdAt asc) {
